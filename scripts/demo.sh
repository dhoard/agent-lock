#!/bin/sh
# demo.sh — see agent-lock working without any manual setup.
#
...
# the verdicts land.
#
#   sudo ~/agent-lock-fresh/scripts/demo.sh           # JAILED — escapes show "blocked"
#   sudo ~/agent-lock-fresh/scripts/demo.sh --audit   # UNCONFINED — escapes show "LEAKED"
#
# Press q to quit the watcher. The real tool is scripts/agent-lock; this just
# wires up activity so there's something to see.
set -eu

HERE=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
JAIL_BIN="$HERE/bin/agent-lock-bin"
...
PROJ=${DEMO_DIR:-/tmp/agent-lock-demo}
...
DECOY_SECRET=/tmp/agent-lock-demo-secret.txt
...
WORKLOAD=/tmp/agent-lock-demo-workload
cat > "$WORKLOAD.sh" <<EOF
n=0
...
# "Text file busy".
STANDIN_DIR=$(mktemp -d /tmp/agent-lock-demo.XXXXXX)
OMP_STANDIN="$STANDIN_DIR/omp"
# Clear any stale standins/processes from earlier demo runs.
pkill -9 -f "/tmp/agent-lock-demo\." 2>/dev/null || true

...
#
#   sudo ~/agent-lock-fresh/scripts/demo-record.sh
#
...
[ -f "$HERE/bin/agent-lock-bin" ] || { echo "run 'make' first"; exit 1; }
...
PROJECT="$PROJ" "$HERE/bin/agent-lock-bin" "$PROJ" -- "$STANDIN" -c "$AGENT" >/dev/null 2>&1 &
WL=$!
cleanup() {
